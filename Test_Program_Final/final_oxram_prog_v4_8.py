...
#       11/04/2016      A.A     V4.6    Main program is included in function test_die(wafer_lot,wafer_number,X_die,Y_die,cut_size,flag)
#       12/04/2016      A.A     V4.7    Add X/Y coordinates in log/bitmap name
#       21/04/2016      A.A     V4.8    Transfert of some parameters from testParams file to testflow.xls
#                                       Add of parameters worksheet in testflow.xls
#                                       Correction of Range mode
#############################################################################################################################################
...
userFile = "variables_testParams_v4_1"
import variables_testParams_v4_1 as var
            ...
            for Wlm in range(startWlm,stopWlm+1):
                for BL in range(startBtl,stopBtl+1):
                    for Wl in range(startWl,stopWl+1):    
                        adrs.append([str(Wlm),str(Wl),str(BL)])
        elif scanMode=="CKBD":
            #noAdrs=noAdrs/2
            for i in range(noAdrs):            
                adrs1=[]
...
def writeResultFile(flag_auto_man,Address_mode,location, test, adrsList, topV, botI, gateV,seqCounts,X_pos,Y_pos):
        if flag_auto_man==1:
            if os.path.exists("%s\\%s_X_%s_Y_%s.csv" % (location,"results",X_pos,Y_pos)) is not True:
                flag=1
            else:
                flag=0
            rslfile = open("%s\\%s_X_%s_Y_%s.csv" % (location,"results",X_pos,Y_pos), "ab")
        else:
...
# create bitmap file
def generateBitmap(flag_auto_man,mtxSize,R, loc,X,Y):
        R2 = []
        if mtxSize=='1M':
            fig, (ax) = plt.subplots()        	
            if flag_auto_man==1:
                pp = PdfPages(r'%s/Bitmap_X_%s_Y_%s.pdf'% (loc,X,Y))         
            else:
            ...
            plt.xlabel('Wordline', fontsize=10)
            if flag_auto_man==1:
                plt.savefig(r'%s/Bitmap_X_%s_Y_%s.pdf'% (loc,X,Y))                            #save bitmap.pdf
            else:
...
def run(flag_auto_man,mtxSize,testFolder,startAdrs,stopAdrs,stopAll,ard,AdrsMode,scanMode,count,singleAdrs,single_address_number,adrsList,seqCounts,testPlan,array_single_address,flag_loop,Test_number,loop_number,X,Y,testName):    
    ...
    ################## LOAD PIV #####################################
            ...
            if testPlan[j].__class__ is ReadOperation:
                ...
                #dieFolderLoc = dieFolder(operationFolder,i+1, X, Y,flag_auto_man)
                dieFolderLoc = operationFolder
                # save results
                ...
                writeResultFile(flag_auto_man,AdrsMode,dieFolderLoc, testName, adrsList, vTopTemp, iBotTemp, vGateTemp,seqCounts,X,Y)
                print "OK"
                if AdrsMode == 'S' and scanMode=="All":
                    # create bitmap
                    print "   Save bitmap...  ",
                    generateBitmap(flag_auto_man,mtxSize,resistance, dieFolderLoc,X,Y)
                    print "OK"
...
def test_die(wafer_lot,wafer_number,X_die,Y_die,cut_size,flag):
    ...
    startBL_R = int(worksheet.cell(9,2).value)
    startWL_R = int(worksheet.cell(10,2).value)
    startSL_R = int(worksheet.cell(11,2).value)
    stopBL_R = int(worksheet.cell(13,2).value)
    stopWL_R = int(worksheet.cell(14,2).value)
    stopSL_R = int(worksheet.cell(15,2).value)   
    ...
    for sequences in range (lines_nb):
        ...
        if AdrsMode == 'A':
            ...
            if scanMode=='All':
                adrsList = adrsTable(scanMode,startBL, startWL, startSL, stopBL, stopWL, stopSL, nbWL[mtxDic.get(mtxSize)], nbBL[mtxDic.get(mtxSize)])
            else:
                adrsList = adrsTable(scanMode,startBL_R, startWL_R, startSL_R, stopBL_R, stopWL_R, stopSL_R, nbWL[mtxDic.get(mtxSize)], nbBL[mtxDic.get(mtxSize)])
            if scanMode in var.patterns:
        ...
        for lp in range(int(loop)):
            ...
            if interval_flag==1:
                if lp in interval_list:
                    ...
                    run(flag,mtxSize,testFolder,startAdrs,stopAdrs,stopAll,ard,AdrsMode,scanMode,counter,singleAdrs,single_address_number,adrsList,seqCounts,testPlan_option_read,array_single_address,1,lines_nb,lp,X_pos,Y_pos,testName)
                    counter=counter+indice_option_read
                    run(flag,mtxSize,testFolder,startAdrs,stopAdrs,stopAll,ard,AdrsMode,scanMode,counter,singleAdrs,single_address_number,adrsList,seqCounts,testPlan,array_single_address,0,0,0,X_pos,Y_pos,testName)
                ...
                counter=counter+indice
        if interval_flag==1:
            run(flag,mtxSize,testFolder,startAdrs,stopAdrs,stopAll,ard,AdrsMode,scanMode,counter,singleAdrs,single_address_number,adrsList,seqCounts,testPlan_option_read,array_single_address,1,lines_nb,int(loop),X_pos,Y_pos,testName)
            counter=counter+indice_option_read
        testPlan=[]
    # wait 20ms
    time.sleep(0.02)
