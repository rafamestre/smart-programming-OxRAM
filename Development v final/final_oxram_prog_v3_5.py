...
from madClassesAndFunctions_v1 import Arduino
...
def address_list():
    fichier = open("address_list.txt", "r")
...
def adrsTable(startBtl, startWl, startWlm, stopBtl, stopWl, stopWlm, stopWlall, stopBtlall):
        adrs = []
...
def writeResultFile(location, test, adrsList, topV, botI, gateV):
        ...
        if var.mtxSize=='1M':
            ...
            for i in range(0,16):
                ...
                for j in range(0,256):     
                    R2.append(R[(i*256+j)*256:((i*256+j)*256)+255])
                image = np.array(R2)
...
if var.adrsMode == 'A':
    ...
    adrsList = adrsTable(startBL, startWL, startSL, stopBL, stopWL, stopSL, nbWL[mtxDic.get(var.mtxSize)], nbBL[mtxDic.get(var.mtxSize)])
...
for i in range(0,single_address_number):
    ...
    for j in range(len(testPlan)):
        ...
        ard.programAddressing(var.adrsMode, singleAdrs, startAdrs, stopAdrs, stopAll)
        print "OK"
...
################## LOAD PIV #####################################
        ...
        operationFolder = r'%s\%s' % (testFolder, "Op"+str(j+1)+"__"+testPlan[j].name)
        if not os.path.exists(operationFolder):
            os.makedirs(operationFolder)
        print "OK"
        # create Parameter file (copy .py param file)
        ...
        if testPlan[j].__class__ is ReadOperation:
            ...
            writeResultFile(dieFolderLoc, var.testName, adrsList, vTopTemp, iBotTemp, vGateTemp)
            print "OK"
            if var.adrsMode == 'S':
                # create bitmap
                print "   Save bitmap...  ",
