...
def writeFiles(path,sheet):
    ...
    [tests, strategies, plan, die, Vs, Vr, stepVs, stepVr, repSet, repReset, date, size, threshold, time, repPlan,order,previous] = readTestFlow(path,sheet)
        ...
        for test in dieTests:
            ...
            if not testPlan[0] == 'form':
                ...
                if sheet > 30:
                    ...
                    print('hereeee')
                    Roncycling = [5000,7000,10000,15000,20000,30000,40000]
                    Roffcycling = [50000,60000,70000,80000,90000,100000,120000]
                    ...
                    for planRep in range(1,2*(int(repePlan)+1)+1):
                        ...
                        for c1 in range(len(thresR)):
                            Working = [0]*int(len(RmeasFloat))
                            for res in range(0,len(RmeasFloat)):

                                if nonWorkingForming[res] == 0 and c1 == 0:
                                    actualR.append(RmeasFloat[res])

                                if planRep%2 == setFolder:
                                        
                                    if RmeasFloat[res] < thresR[c1] and nonWorkingForming[res] == 0:
                                        Working[int(res)] = 1
                            ...
                            totalWorking = Working.count(1)
                                    
                            totalCells = int(len(RmeasFloat)-nonWorkingForming.count(1))
                            
                            meanR = np.mean(actualR)
                            stdevR = np.std(actualR)
                                    
                            if planRep%2 == setFolder:
                                fileName = 'cycling_set'
                                thres = Roncycling[c1]
                                volt = Vs[test].value
                                fileMean = 'mean_set'
                            else:
                                fileName = 'cycling_reset'
                                thres = Roffcycling[c1]
                                volt = Vr[test].value
                                fileMean = 'mean_reset'

                            print(fileName)
                            print(planRep)
                            operationNb = order[test].value+planRep-1
                            print(str(totalCells))
                            ...
                            if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileName+'.txt') is False:
                                ...
                                wr.write('before\tafter\ttotal\tV1\tV2\trep\tdate\ttest\tth\tt\top\tpre\n')
                                wr.write(str(totalWorkingBefore)+'\t'+str(totalWorking)+'\t'+str(totalCells)+'\t'+\
                                         str(volt)+'\t'+str(Vbefore)+\
                                         '\t'+str(0)+'\t'+theDate+'\t'+str(int(tests[test].value))+\
                                         '\t'+str(int(thres))+'\t'+str(time[test].value)+'\t'+str(int(operationNb))+'\t'+preOp+'\n')
                                wr.close()
                            else:
                            ...
                            if (size[test].value == '4k' or size[test].value == '4K') and c1==0:

                                if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileMean+'.txt') is False:
                                    wr = open(path+'\\Mixed\\efficiencies\\'+fileMean+'.txt','w')
                                    wr.write('before\tafter\ttotal\tV1\tV2\trep\tdate\ttest\tth\tt\top\tpre\tmean\tstdev\n')
                                    wr.write(str(totalWorkingBefore)+'\t'+str(totalWorking)+'\t'+str(totalCells)+'\t'+\
                                             str(volt)+'\t'+str(Vbefore)+\
                                             '\t'+str(0)+'\t'+theDate+'\t'+str(int(tests[test].value))+\
                                             '\t'+str(int(thres))+'\t'+str(time[test].value)+'\t'+str(int(operationNb))+'\t'+preOp+\
                                             '\t'+str(meanR)+'\t'+str(stdevR)+'\n')
                                    wr.close()
                                else:
                                    wr = open(path+'\\Mixed\\efficiencies\\'+fileMean+'.txt','a')
                                    wr.write(str(totalWorkingBefore)+'\t'+str(totalWorking)+'\t'+str(totalCells)+'\t'+\
                                             str(volt)+'\t'+str(Vbefore)+\
                                             '\t'+str(0)+'\t'+theDate+'\t'+str(int(tests[test].value))+\
                                             '\t'+str(int(thres))+'\t'+str(time[test].value)+'\t'+str(int(operationNb))+'\t'+preOp+\
                        ...
                        totalWorkingBefore = Working.count(1)
                        if planRep%2 == setFolder:
                            preOp = 'set'
                            Vbefore = Vs[test].value
                        else:
                            preOp = 'reset'
                            Vbefore = Vr[test].value
            


                elif strategies[test].value == 'TB':
                    ...
                    if size[test].value == '64k' or size[test].value == '1M':
                        ...
                        for planRep in range(1,2*(int(repePlan)+1)+1):
                            ...
                            for repe in range(repetitions):
                                ...
                                mypath2 = mypath+'\\'+operationFolder+'\\x0_y0\\'
                                if os.path.exists(mypath+'\\'+operationFolder) == True:
                                    Rmeas, SL, WL, BL = readR(mypath,operationFolder)
                                else:
                                    print('All repetitions were achieved')
                                    break
                                print(mypath2)
                            ...
                            #if workingCellsAfter[-1] > workingCellsBefore:
                            if True:        
                                if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileName+'.txt') is False:
                                    wr = open(path+'\\Mixed\\efficiencies\\'+fileName+'.txt','w')
                                    wr.write('before\tafter\ttotal\tV1\tV2\trep\tdate\ttest\tth\tt\top\tpre\n')
                                    wr.write(str(workingCellsBefore)+'\t'+str(workingCellsAfter[-1])+'\t'+str(totalCells)+'\t'+\
                                             str(Vs[test].value)+'\t'+str(Vr[test].value)+\
                        ...
                        for repe in range(repetitions):
                            ...
                            mypath2 = mypath+'\\'+operationFolder+'\\x0_y0\\'
                            if os.path.exists(mypath2) == True:
                                    Rmeas, SL, WL, BL = readR(mypath,operationFolder)
                            else:
                                print('All repetitions were achieved')
                                break
...
if '__main__'==__name__:
    ...
    while not stop:
        try:
            sheet = raw_input('Insert sheet number:\n')
        except:
            sheet = input('Insert sheet number:\n')

        stop = True

    sheet = int(sheet)
    writeFiles(path,sheet)
