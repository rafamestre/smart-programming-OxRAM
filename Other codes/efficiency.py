                ...
                if strategies[test].value == 'TB':
                    ...
                    if size[test].value == '64k' or size[test].value == '1M':
                        ...
                        for planRep in range(1,2*(int(repePlan)+1)+1):
                            ...
                            if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileName+'.txt') is False:
                                ...
                                wr.write('Before \tAfter\tV set\tV reset\tRepetitions\tDate\tTest number\tThreshold\n')
                                wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vs[test].value)+'\t'+str(Vr[test].value)+\
                                         '\t'+str(repetitions-1)+'\t'+theDate+'\t'+str(int(tests[test].value))+'\t'+str(int(thres))+'\n')
                                wr.close()
                            else:
                        ...
                        if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileName+'.txt') is False:
                            wr = open(path+'\\Mixed\\efficiencies\\'+fileName+'.txt','w')
                            wr.write('Before \tAfter\tV set\tV reset\tRepetitions\tDate\tTest number\tThreshold\n')
                            wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vs[test].value)+'\t'+\
                                     str(Vr[test].value)+'\t'+str(repetitions-1)+'\t'+theDate+'\t'+str(int(tests[test].value))+'\t'+str(int(thres))+'\t\n')
                            wr.close()
                        else:
                            wr = open(path+'\\Mixed\\efficiencies\\'+fileName+'.txt','a')
                            wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vs[test].value)+'\t'+\
                    ...
                    if strategies[test].value == 'V' and size[test].value == '64k' and len(testPlan) > 1:
                        ...
                        for planRep in range(1,2*(int(repePlan)+1)+1):
                            ...
                            if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileName+'.txt') is False:
                                wr = open(path+'\\Mixed\\efficiencies\\'+fileName+'.txt','w')
                                wr.write('Before \tAfter\tV initial\tV final\tRepetitions\tRepetitions\tDate\tTest number\tThreshold\n')
                                wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vinit)+'\t'+\
                                         str(Vfin)+'\t'+str(repetitions-1)+'\t'+theDate+'\t'+str(int(tests[test].value))+'\t'+str(int(thres))+'\n')
                                wr.close()
                            else:    
                                wr = open(path+'\\Mixed\\efficiencies\\'+fileName+'.txt','a')
                                wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vinit)+'\t'+\
                        ...
                        if strategies[test].value == 'V':
                            ...
                            if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileName+'.txt') is False:
                                ...
                                wr.write('Before \tAfter\tV initial\tV final\tRepetitions\tRepetitions\tDate\tTest number\tThreshold\n')
                                wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vinit)+'\t'+\
                                         str(Vfin)+'\t'+str(repetitions)+'\t'+theDate+'\t'+str(int(tests[test].value))+'\t'+str(int(thres))+'\n')
                                wr.close()
                            else:    
                                wr = open(path+'\\Mixed\\efficiencies\\'+fileName+'.txt','a')
                                wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vinit)+'\t'+\
                            ...
                            if os.path.isfile(path+'\\Mixed\\efficiencies\\'+fileName+'.txt') is False:
                                wr = open(path+'\\Mixed\\efficiencies\\'+fileName+'.txt','w')
                                wr.write('Before \tAfter\tV initial\tV smart\tRepetitions\tDate\tTest number\tThreshold\n')
                                wr.write(str(effBefore)+'\t'+str(effAfter[-1])+'\t'+str(Vinit)+'\t'+\
                                         str(Vsmart)+'\t'+str(repetitions)+'\t'+theDate+'\t'+str(int(tests[test].value))+'\t'+str(int(thres))+'\n')
                                wr.close()
                            else:
