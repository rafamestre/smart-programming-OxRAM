...
class SmartStrategy:
        def __init__(self, strategy, operation, step, maxRep, threshold, read):
...
class Arduino:
        ...
        def programAddressing(self, adrsMode, adrsSingle, adrsList, startAdrs, stopAdrs, stopAll):
                # get addresses
                self.adrsMode = adrsMode
                self.adrsSingle = adrsSingle
                self.adrsList = adrsList
                ...
                if self.adrsMode == 'A':
                        ...
                        while ser.read() != '0':
                                ser.read()
                elif self.adrsMode == 'L':
                        # this particular case would lead to Arduino buffer overflow (limited to 64 bytes only Oo)
                        # thus only 5 addresses are sent at a time (max size = xxx/xxx/xxx/ = 12 bytes)
                        # after having sent 5 addresses, wait for Arduino reply
                        i=0
                        for x in adrsList:
                                ser.write(self.adrsMode) #WHY?????
                                ser.write(x)
                                if (i % 20) == 0:
                                        print('\n      Address ' + str(i*5)+ ' sent \n')
                                while ser.read() != '0':
                                        ser.read()
                                i+=1
                                if i == 275:
